...
class ResBlocksPomelo(nn.Module):
    ...
    '''
    def __init__(self, input_channels, feature_dim,  classifier="v1", head="v1", down=5,
                occupancymodel=False):
        super(ResBlocksPomelo, self).__init__()
        """
        Args:
        ...
        self.params_sum = sum(p.numel() for p in self.backmodel.parameters() if p.requires_grad)
        print("Number of parameters in the backbone: {}".format(self.params_sum))
    ...
    def forward(self, inputs, train=False, padding=True, alpha=0.1, return_features=True, encoder_no_grad=False, unet_no_grad=False):
        ...
        if  unet_no_grad:
            with torch.no_grad():
                features, decoder_features = self.backmodel(data, return_features=return_features, encoder_no_grad=encoder_no_grad)
        else:
            features, decoder_features = self.backmodel(data, return_features=return_features, encoder_no_grad=encoder_no_grad)
